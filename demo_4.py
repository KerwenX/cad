...
logging.basicConfig(filename='demo_test.out.log', level=logging.INFO, filemode='w',format=log_format)
...
def get_args_parser():
    ...
    parser.add_argument('--test_ckpt_addr', default='/home/aston/Desktop/python/Test/CADMatching-test/20230626/checkpoint_best.pt', help='the ckpt to test 3d zero shot')
...
def main(args):
    total_file_number = 0 # total file number 
    image_correct_top1 = 0 # image retrieval instance top1 accuracy
    image_correct_top5 = 0 # image retrieval instance top5 accuracy
    text_correct_cat_top1 = 0 # cat acc
    text_correct_cat_top10 = 0
    # common_correct = 0 
    cat_image_acc_top1 = {} # each category accuracy
    ...
    # pc_encoder
    with open('ULIP_PointBERT_cads_feature.pkl', 'rb') as f:
    ...
    with torch.no_grad():
        for dirname in dirname_list:
            ...
            for image_path in image_path_list:
                ...
                _,text_retrival_top10 = torch.topk(logits_per_text_pc,k=10)
                
                pc_for_image_feature = [pc_embed_all[index] for index in text_retrival_top10[0]]
                pc_for_image_feature = torch.stack(pc_for_image_feature)
                logits_per_image_pc = logit_scale * image_embed @ pc_for_image_feature.t()
                _, image_retrival = torch.topk(logits_per_image_pc, k=1)

                image_pc_retrieval_namelist = [filename_list[text_retrival_top10[0][image_retrival[0]]]]
                text_pc_retrieval_namelist = [filename_list[index] for index in text_retrival_top10[0]]
                ...
                if id_cad in image_pc_idcad_retre:
                    image_correct_top5+=1
                    if id_cad == image_pc_idcad_retre[0]:
                        image_correct_top1+=1
                        cat_image_acc_top1[caption]+=1
                if catid_cad in text_pc_catid_retre:
                    text_correct_cat_top10+=1
                    if catid_cad == text_pc_catid_retre[0]:
                        text_correct_cat_top1+=1
                ...
                if total_file_number%50==0:
                    logging.info(
                        ...
                        "Image_top1_acc: {:.3f}%, Image_top5_acc: {:.3f}%, Text_cat_top1_acc: {:.3f}%, Text_cat_top10_acc: {:.3f}%. ".format(
                            image_correct_top1 * 100 / total_file_number,
                            image_correct_top5 * 100 / total_file_number,
                            text_correct_cat_top1 * 100 / total_file_number,
                            text_correct_cat_top10 * 100 / total_file_number,
                        )
                    )
    logging.info(
        "[RESULT] Image_top1_acc: {}%, Image_top5_acc: {}%, Text_cat_top1_acc: {}%, Text_cat_top5_acc: {}%".format(
            image_correct_top1*100/total_file_number,
            image_correct_top5*100/total_file_number,
            text_correct_cat_top1*100/total_file_number,
            text_correct_cat_top10*100/total_file_number,
...
# retrieval CAD with text first
# then retrieval instance within the former result
