...
log_format = '%(asctime)s - DEMO - only image with GT cat - %(levelname)s - %(message)s'
logging.basicConfig(filename='demo_only_image_GT_cat.out.log', level=logging.INFO, filemode='a',format=log_format)
...
def get_args_parser():
    ...
    parser.add_argument('--test_ckpt_addr', default='/home/aston/Desktop/python/cad-match/outputs/reproduce_pointbert_8kpts_only_image/checkpoint_last.pt', help='the ckpt to test 3d zero shot')
...
def main(args):
    ...
    # pc_encoder
    with open('ULIP_PointBERT_cads_feature_only_image.pkl', 'rb') as f:
    ...
    with torch.no_grad():
        for dirname in dirname_list:
            ...
            for image_path in image_path_list:
                ...
                if len(cat_cad_index_list)<top_k:
                    total_file_number-=1
                    continue
                ...
                _, image_retrival_top5 = torch.topk(logits_per_image_pc, k=top_k)

                image_pc_retrieval_namelist = [filename_list[cat_cad_index_list[index]] for index in image_retrival_top5[0]]
                ...
                if total_file_number%50==0:
                    logging.info("[{}-{}][{}]".format(catid_cad, id_cad, caption) +
                        " Image_top1_acc: {:.1f}%, Image_top5_acc: {:.1f}%. ".format(
