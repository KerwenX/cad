...
class MyTest(data.Dataset):
    def __init__(self, config):
        ...
        self.catfile = os.path.join(self.root, 'shape_name.txt')

        self.cat = [line.rstrip() for line in open(self.catfile)]
        self.classes = dict(zip(self.cat, range(len(self.cat))))

        assert (split == 'train' or split == 'test')
        # print_log('The size of %s data is %d' % (split, len(self.datapath)), logger='MyTest')

        self.save_path = os.path.join(self.root,'mydata_test_8192pts_fps.dat')

        if self.process_data:
            print_log('Load processed data from %s...' % self.save_path, logger='MyTest')
            with open(self.save_path, 'rb') as f:
                self.list_of_points, self.list_of_labels = pickle.load(f)

        self.shape_names_addr = os.path.join(self.root, 'shape_name.txt')
        with open(self.shape_names_addr) as file:
            lines = file.readlines()
            lines = [line.rstrip() for line in lines]
        self.shape_names = lines
