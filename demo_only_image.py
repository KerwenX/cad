...
log_format = '%(asctime)s - DEMO - only image - %(levelname)s - %(message)s'
logging.basicConfig(filename='demo_only_image.out.log', level=logging.INFO, filemode='a',format=log_format)
...
def main(args):
    ...
    with torch.no_grad():
        for dirname in dirname_list:
            ...
            for image_path in image_path_list:
                ...
                #         logging.info('Encodering pc {}'.format(filename))
                #     pc_embed_all = torch.stack(pc_embed_all)
                #     pc_encoder_label = True
                ...
                if total_file_number%50==0:
                    logging.info("[{}-{}][{}]".format(catid_cad, id_cad, caption) +
                        " Image_top1_acc_instance: {:.1f}%, Image_top5_acc_instance: {:.1f}%. ".format(
    ...
    logging.info(
        "[RESULT] Image_top1_acc_instance: {}%, Image_top5_acc_instance: {}%".format(
            image_correct_top1*100/total_file_number,
            image_correct_top5*100/total_file_number,
            # text_correct_top1*100/total_file_number,
            # text_correct_top5*100/total_file_number,
...
# retrieval CAD from only image alignment
