...
log_format = '%(asctime)s - DEMO5 - %(levelname)s - %(message)s'
logging.basicConfig(filename='demo_5.out.log', level=logging.INFO, filemode='a',format=log_format)
...
def main(args):
    ...
    with torch.no_grad():
        for dirname in dirname_list:
            ...
            for image_path in image_path_list:
                ...
                if catid_cad in image_pc_catid_retre[0]:
    ...
    logging.info(
        "[RESULT] Image_top1_acc_cat: {}%".format(
            image_correct_top1*100/total_file_number,
            # image_correct_top5*100/total_file_number,
            # text_correct_top1*100/total_file_number,
            # text_correct_top5*100/total_file_number,
...
# using image to retrieve the same category CAD Models
