...
class ULIP_WITH_IMAGE(nn.Module):
    ...
    def forward(self, pc, text, image=None):
        ...
        pc_embed = self.encode_pc(pc) # b*512
        if image is not None:
            image_embed = self.encode_image(image) # b*512
...
def ULIP_PointBERT(args):
    ...
    config_addr = '/home/aston/Desktop/python/CAD-Matching/models/pointbert/PointTransformer_8192point.yaml'
    ...
    if not args.evaluate_3d:
        # load the pretrained model
        pretrain_slip_model = torch.load('/home/aston/Desktop/python/CAD-Matching/data/initialize_models/slip_base_100ep.pt', map_location=torch.device('cpu'))
        pretrain_slip_model_params = pretrain_slip_model['state_dict']
        pretrain_slip_model_params = {param_name.replace('module.', ''): param for param_name, param in
                                      pretrain_slip_model_params.items()}
